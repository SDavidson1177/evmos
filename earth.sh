...
if [ $CMD = "start" ]; then
    # Usage
    USAGE="Usage: ./earth.sh start <number of nodes> <ip address of node 0> [OPTIONS]\n
    \tOptions:\n
    ...
    docker container stop $(cat ./.memory.txt)
    docker container remove $(cat ./.memory.txt)
else
